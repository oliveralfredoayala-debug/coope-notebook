...
| 3 | El modelo cooperativo | Introducción · Cooperativa vs. empresa mercantil · El modelo institucional cooperativo (North) · El conflicto de la cooperativa · Qué se hace con la ganancia · Para llevarte | El modelo cooperativo (parent) | **package_ready (v1.0)** | Manual 2020, Art. 4 LCH, CURSO_BASICO_2020.pptx slide 22, North vía chat previo |
...
Claude produjo por error un mockup visual (widget con tabs, colores, tarjetas
estilizadas) para "mostrar el HTML" de la Cápsula 2. ESTO FUE UN ERROR. La
navegación real es: tabs siempre a la izquierda, sin menú superior (eso lo
maneja GPT/HTML Coordinator). Claude NUNCA debe producir mockups visuales ni
asumir layout — cuando se pida ver el HTML, mostrar el código semántico crudo
tal cual, sin renderizar ni diseñar nada.

## SKILL v3.1 — REEMPLAZA v2 por completo (2026-06-30)

Archivo: SKILL-content-author-v3.1.md (subir a project knowledge, reemplaza
SKILL-content-author-v2.md que queda obsoleto).

Cambios críticos respecto a v2:
- **PROHIBIDO inner_tabs / subtabs / navegación anidada, sin excepción.** Toda
  cápsula usa lista PLANA de tabs de primer nivel. Si un tema tiene 5
  secciones, son 5 tabs planas, nunca un tab padre que las contenga.
- Jester: Claude entrega SOLO texto (`opening_thought`), nunca HTML/div/clases
  de Jester. El Coordinator compone visualmente el personaje.
- HTML permitido: solo section/h2/h3/p/ul/ol/li/blockquote/table/strong/em.
  PROHIBIDO: style, class, script, botones, tabs, acordeones, textarea/input,
  cualquier control interactivo.
- Contenido legal: usar formato `legal_excerpt` (source/article/mode/text),
  no acordeones ni clases.
- `presentation_hint` reemplaza "propuesta_visual": debe describir la
  NECESIDAD COGNITIVA, nunca nombrar un componente ("poner en acordeón" está
  prohibido; "conviene que se puedan leer juntas y relacionar" sí).
- Banco de preguntas: sigue siendo archivo separado, formato igual que antes.
- Campos prohibidos explícitos: inner_tabs, subtabs, nested_navigation,
  carousel_navigation, shell_override, layout, theme, color_palette,
  component. Si aparecen, el Source Package se rechaza.
- El HTML Coordinator tiene AUTORIDAD FINAL sobre diseño/navegación/shell;
  mis notas son sugerencias no vinculantes, nunca órdenes.

## ACCIÓN PENDIENTE: Cápsula 2 (Identidad cooperativa v2) VIOLA la skill v3.1
Usa inner_tabs (prohibido) y HTML con clases/textarea (prohibido). Debe
reconstruirse en tabs planas antes de considerarse package_ready. Pendiente
de hacer en la próxima sesión si Oliver lo confirma.

## Cápsula 3 — preview aprobado con ajuste (2026-06-30)
Título: El modelo cooperativo. Parent propio: "El modelo cooperativo".
Tabs planas (skill v3.1): Introducción · Cooperativa vs. empresa mercantil ·
El modelo institucional cooperativo (marco de North, adaptación interpretativa,
reutilizado de chat previo "Modelo cooperativo y empoderamiento institucional")
· El conflicto de la cooperativa · Qué se hace con la ganancia (breve, remite
al artefacto separado Explorador de excedentes, sin duplicar) · Para llevarte.
Ajuste de Oliver: opening_thought corregido a "pueden parecer lo mismo" en vez
de "pueden vender lo mismo" — más preciso, no compara solo el producto sino
la percepción externa. Pendiente: producir Source Package + banco de preguntas.

## Cápsula 3 — FINAL, package_ready (2026-06-30)
Título: El modelo cooperativo. Parent propio: "El modelo cooperativo".
Skill v3.1 (tabs planas, sin inner_tabs, sin HTML con clases/estilos,
opening_thought en vez de jester-piensa HTML, presentation_hint no vinculante,
sources trazables). Archivos: capsula-03-el-modelo-cooperativo.yaml (Source
Package), capsula-03-banco-preguntas.yaml (9 preguntas).
Reutiliza contenido del marco de Douglass North del chat previo "Modelo
cooperativo y empoderamiento institucional" (12 jun 2026), marcado
explícitamente como adaptación interpretativa.

## Reorden de producción (2026-06-30)
Oliver adelanta "Clasificación de las cooperativas" antes que "Derechos y
deberes del afiliado/a" y "Estructura orgánica". Orden de PRODUCCIÓN pasa a
ser: 1 Orígenes ✓ · 2 Identidad ✓ · 3 El modelo ✓ · 4 Clasificación de las
cooperativas (siguiente) · pendientes: Derechos y deberes, Estructura
orgánica, Integración, Símbolos, Contexto actual. El orden dentro del Parent/
Jester no cambia, solo el orden en que Claude las produce.

## Cápsula "Clasificación de las cooperativas" — FINAL, package_ready (2026-06-30)
Parent: Identidad cooperativa. Tabs planas (skill v3.1): Introducción · Por
tipo de actividad (Art. 47-51 LCH, Art. 29 RLCH) · Por grado de integración
(Art. 88 LCH, Art. 149 RLCH) · Por afiliación abierta o cerrada (caso ELGA,
sin artículo específico verificado — marcado para revisión futura) · Para
llevarte. Se descartó el eje "por tipo de excedentes" por indicación de
Oliver: no es una clasificación real, es consecuencia operativa ya cubierta
en el artefacto de excedentes.
Archivos: capsula-clasificacion-de-las-cooperativas.yaml (Source Package),
capsula-clasificacion-banco-preguntas.yaml (8 preguntas).

## Cápsula "Derechos y deberes del afiliado/a" — FINAL, package_ready (2026-06-30)
Parent: Identidad cooperativa. Tabs planas: Introducción · Deberes
cooperativos (Art. 75 LCH literal + 4 preguntas de reflexión aprobadas por
Oliver, fusionadas de 7 candidatas, sin la de "promover ingreso de nuevos
cooperativistas") · Derechos cooperativos (Art. 76 LCH literal) · Para
llevarte. Archivos: capsula-derechos-y-deberes-del-afiliado.yaml (Source
Package), capsula-derechos-y-deberes-banco-preguntas.yaml (5 preguntas).

## Cápsula "Integración del movimiento cooperativo" — PAUSADA (2026-06-30)
Preview no convenció a Oliver del todo (no dijo por qué exactamente). Contenido
trabajado hasta ahora: Sexto Principio (Art. 8 RLCH) como fundamento, CHC
(Art. 90-91 LCH), CONSUCOOP (Art. 93, 93-B LCH) diferenciado explícitamente de
la CHC, listado de 11 organismos de segundo grado. Traslape menor identificado
con el tab "Por grado de integración" de la cápsula Clasificación (solo el dato
FACACH=segundo grado / CHC=tercer grado se repite). Retomar al final de la
serie. NO producir Source Package todavía.

Nota: pendiente también producir versión "beginner" (Diplomado Básico) de
Estructura orgánica, ya que la existente (chat "Tensiones del mapa de poder
cooperativo") es de nivel más avanzado.

## Cápsula "Símbolos cooperativos" — DESCARTADA (2026-06-30)
Oliver confirma que es redundante: ya cubierta suficientemente en el inner tab
3 "Lo que nos representa" de la cápsula Identidad cooperativa. Se elimina del
plan de producción.

## Cápsula "Integración del movimiento cooperativo" — FINAL, package_ready (2026-06-30)
Retomada tras pausa. Parent: Identidad cooperativa. Tabs planas: Introducción
· Integración por niveles (Art. 88 LCH) · La CHC como organismo superior
(Art. 90-91 LCH) · CONSUCOOP, el organismo supervisor (Art. 93 LCH,
diferenciado explícitamente de la CHC) · Organismos de segundo grado (11
listados) · Para llevarte. Se descartó un tab "Qué es integrarse" (Sexto
Principio) por decisión de Oliver: aporte "blando", no indispensable.
Se descartó el dato de Art. 93-B (nombramiento del Director Ejecutivo) como
apertura por no ser la base legal explícita de integración por niveles.
Confirmado: el traslape con Art. 88 LCH ya usado en "Clasificación de las
cooperativas" es intencional, cada cápsula es standalone y no se referencian
entre sí. Archivos: capsula-integracion-del-movimiento.yaml (Source Package),
capsula-integracion-banco-preguntas.yaml (7 preguntas).
